...
class ModelCheckpoint:
    def __init__(self):
        self.best_models = []  # 전체 상위 3개 모델
        self.best_models_post_35 = []  # 35 에포크 이후 상위 3개 모델
        self.high_dice = 0.0
        self.high_dice_post_35 = 0.0  # 35 에포크 이후 최고 dice
    ...
    def delete_ckpt(self, model_list):
        dice_remove, epoch_remove, path_remove = model_list.pop(-1)
    ...
    def save_model(self, model, epoch, loss, dice):
        ...
        if len(self.best_models) < args.num_ckpt:
            ...
            self.delete_ckpt(self.best_models)
            self.save_ckpt(model, current_path)
            self.best_models.append((dice, epoch, current_path))
            print(f"Save model for epoch {epoch+1} with dice = {dice:.4f}")
        ...
        if epoch + 1 > 35:
            self.save_model_post_35(model, epoch, loss, dice)

    def save_model_post_35(self, model, epoch, loss, dice):
        model_file_name = f'post35_epoch_{epoch+1}_dice_{dice:.4f}_loss_{loss:.4f}.pt'
        current_path = os.path.join(args.saved_dir, model_file_name)

        # 35 에포크 이후 상위 3개 관리 (독립적)
        if len(self.best_models_post_35) < args.num_ckpt:
            self.save_ckpt(model, current_path)
            self.best_models_post_35.append((dice, epoch, current_path))
        elif dice > self.best_models_post_35[-1][0]:
            self.delete_ckpt(self.best_models_post_35)
            self.save_ckpt(model, current_path)
            self.best_models_post_35.append((dice, epoch, current_path))

        if dice > self.high_dice_post_35:
            print(f"Post-35 Best performance at epoch: {epoch + 1}, {self.high_dice_post_35:.4f} -> {dice:.4f}")
            self.high_dice_post_35 = dice

        # 정렬하여 가장 높은 dice가 먼저 오도록 유지
        self.best_models_post_35.sort(reverse=True)
    ...
    def save_best_model(self, model):
        ...
        # 35 에포크 이후 상위 모델 저장
        if self.best_models_post_35:
            best_post35_model_path = os.path.join(args.saved_dir, 'best_post35_model.pt')
            best_dice_post35, best_epoch_post35, _ = self.best_models_post_35[0]
            torch.save(model, best_post35_model_path)
            print(f"Best post-35 model saved for epoch {best_epoch_post35+1} with highest dice = {best_dice_post35:.4f}")
...
def adjust_curriculum(epoch):
    if epoch < 35:
        train_dataset = XRayDataset(image_root=cf.TRAIN_IMAGE_ROOT1, label_root=cf.TRAIN_LABEL_ROOT1, is_train=True, transforms = cf.transforms['train'])
    elif epoch >= 35:
        train_dataset = XRayDataset(image_root=cf.TRAIN_IMAGE_ROOT2, label_root=cf.TRAIN_LABEL_ROOT2, is_train=True, transforms = cf.transforms['train'])
...
def train(args):
    ...
    valid_dataset = XRayDataset(
        image_root=cf.VAL_IMAGE_ROOT_C,
        label_root=cf.VAL_LABEL_ROOT_C,
        is_train=False, 
        transforms = val_tf, 
        )
    ...
    for epoch in range(args.num_epochs):
        model.train()
        train_loader = adjust_curriculum(epoch)
    ...
    # 마지막 에포크에서 모델 저장
    print("Final epoch completed. Saving the final model...")
    final_model_path = os.path.join(args.saved_dir, f'final_epoch_{args.num_epochs}.pt')
